...
dataset_dir = 'datasets'  # change to where you store datasets
...
class ModelNetSo3():
    def __init__(self, path=dataset_dir, category=None):
        self.modelnet_dir = os.path.join(path, 'ModelNet10-SO3')
        self.category = category

    def get_train(self):
        full = ModelNetSo3Subset(os.path.join(self.modelnet_dir, train_name), category=self.category)
        return full

    def get_eval(self):
        return ModelNetSo3Subset(os.path.join(self.modelnet_dir, test_name), category=self.category)
...
class ModelNetSo3Subset():
    def __init__(self, path, imsize=224, category=None):
        self.path = path
        ...
        for i in range(len(keys)):
            if category is not None:
                flag = keys[i].find(category) >= 0
            else:
                flag = True
            if flag:
                self.keys.append(keys[i])
                self.quaterions.update({keys[i]: quaterions[keys[i]]})
                self.euler.update({keys[i]: euler[keys[i]]})
        print(len(self.keys))

        self.bits = []
        with self.lmdb.begin() as txn:
            for key in self.keys:
                temp_bits = txn.get(key.encode('utf8'))
                self.bits.append(temp_bits)
